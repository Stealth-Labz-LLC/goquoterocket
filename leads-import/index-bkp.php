...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    if ($triggerTrackapi == 1) {
        $updateLeadData = "UPDATE auto_rocket SET track_payload='$track_payload',track_response= '$track_response',triggerTrackapi='$triggerTrackapi',track_preferredtimeofcall='$track_preferredtimeofcall' WHERE phone1='$phone1'";
    ...
    if ($triggerWarrantyapi == 1) {
        $updateLeadData = "UPDATE auto_rocket SET warranty_payload='$warranty_payload',warranty_response='$warranty_response',triggerWarrantyapi='$triggerWarrantyapi',warranty_preferredtimeofcall='$warranty_preferredtimeofcall' WHERE phone1='$phone1'";
        //file_put_contents('./updateleadlog_View' . time() . '.log',$updateLeadData, FILE_APPEND);
        if ($conn->query($updateLeadData) === TRUE) {
            $LeadResponse = array(
                "status" => "SUCCESS",
