...
$endpoints["addData"] = function (array $requestData): void {
    ...
    $sid = $requestData['sid'];
    $debt_payload = $requestData['debt_payload'];
    $debt_response = $requestData['debt_response'];
    ...
    $check_duplicate = "SELECT id FROM debt_rocket WHERE phone1='$phone1'";
    $result = $conn->query($check_duplicate);

    if ($result->num_rows > 0) {
        // Record exists, perform UPDATE
        $query = "UPDATE debt_rocket SET  debt_payload='$debt_payload',debt_response='$debt_response' WHERE phone1='$phone1'";
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    } else {
        // Record does not exist, perform INSERT
        $query = "INSERT INTO debt_rocket (funnel_id,sid,aff_sub,aff_sub2,aff_sub3,debt_payload,debt_response,city,optindate,optinurl,underdebtreview,firstname,lastname,phone1,acceptterms,offer_id,preferredtimeofcall)
    VALUES('$funnel_id','$sid','$aff_sub','$aff_sub2','$aff_sub3','$debt_payload','$debt_response','$city','$optindate','$optinurl','$underdebtreview','$firstname','$lastname','$phone1','$acceptterms','$offer_id','$preferredtimeofcall')";
        //echo $query;
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    }
    //END
...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    if ($dripcellContactUpload == 1) {
        $updateLeadData = "UPDATE debt_rocket SET dripcell_payload='$dripcell_payload',dripcell_response='$dripcell_response',dripcellContactUpload='$dripcellContactUpload' WHERE phone1='$phone1'";
