...
try {
    ...
    $query = "SELECT id, phone1, auto_payload FROM track_rocket WHERE executeAutoApiAt <= '$currentTime' AND scheduleAutoApi = '1' AND isAutoApiFired = '0'";
    // echo $query;
    // die();
    ...
    while ($task = $result->fetch_assoc()) {
        try {
            $autoPayload = json_decode($task['auto_payload'], true);
            $phone1 = $task['phone1'];
            autoApiCall($autoPayload, $phone1, $connectionData);
        } catch (Exception $e) {
            $updateQuery = "UPDATE track_rocket SET error_log = 'Function Failed' WHERE phone1 = " . $phone1;
        }
    }
    $result->free();
    $conn->close();
...
function AutoApiCall($autoPayload, $phone1, $connectionData)
{
    try {
        ...
        $autoCurl = curl_init();
        curl_setopt_array($autoCurl, [
            CURLOPT_URL => 'https://returnxdigital.leadbyte.co.uk/api/submit.php',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS => http_build_query($autoPayload),
            CURLOPT_HTTPHEADER => ['Content-Type: application/x-www-form-urlencoded']
        ]);
        $autoResponse = curl_exec($autoCurl);
        curl_close($autoCurl);
        $autoResponseData = json_decode($autoResponse);
        // Lead import API
        if ($autoResponseData->code !== -2) {
            $leadUpdateData = [
                'token' => AUTH_TOKEN,
                'phone1' => $phone1,
            ...
            curl_setopt_array($leadUpdateCurl, [
                CURLOPT_URL => 'https://quoterocket.co.za/leads-import/track/updateAutoData',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_CUSTOMREQUEST => 'POST',
        ...
        // $updateQuery = "UPDATE track_rocket SET isAutoApiFired = '1' WHERE id = " . $phone1;
        // if (!$conn->query($updateQuery)) {
        //     $updateQuery = "UPDATE track_rocket SET error_log = '$conn->error' WHERE phone1 = " . $phone1;
        // }
    } catch (Exception $e) {
