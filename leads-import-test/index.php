...
$endpoints["addData"] = function (array $requestData): void {
    ...
    $sid = $requestData['sid'];
    $insurance_payload = $requestData['insurance_payload'];
    $insurance_response = $requestData['insurance_response'];
    ...
    $currentinsurer = $requestData['currentinsurer'];
    ...
    $currentlyinsured = $requestData['currentlyinsured'];
    ...
    $ins_preferredtimeofcall = $requestData['ins_preferredtimeofcall'];

    // Check if the record exists
    $check_duplicate = "SELECT id FROM auto_rocket_test WHERE phone1='$phone1'";
    $result = $conn->query($check_duplicate);

    if ($result->num_rows > 0) {
        // Record exists, perform UPDATE
        $query = "UPDATE auto_rocket_test SET  insurance_payload='$insurance_payload',insurance_response='$insurance_response' WHERE phone1='$phone1'";
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    } else {
        // Record does not exist, perform INSERT
        $query = "INSERT INTO auto_rocket_test (funnel_id,sid,aff_sub,aff_sub2,aff_sub3,insurance_payload,insurance_response,city,firstname,lastname,phone1,optinurl,optindate,currentinsurer,vehiclemake,vehiclemodel,vehicleyear,mileage,currentcarrier,currentlyinsured,employmentstatus,income,acceptterms,offer_id,ins_preferredtimeofcall)
    VALUES('$funnel_id','$sid','$aff_sub','$aff_sub2','$aff_sub3','$insurance_payload','$insurance_response','$city','$firstname','$lastname','$phone1','$optinurl','$optindate','$currentinsurer','$vehiclemake','$vehiclemodel','$vehicleyear','$mileage','$currentcarrier','$currentlyinsured','$employmentstatus','$income','$acceptterms','$offer_id','$ins_preferredtimeofcall')";
        //echo $insertQuery;
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    }
    //END
...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    $phone1 = $requestData['phone1'];
    $track_payload = !empty($requestData['track_payload']) ? $requestData['track_payload'] : '';
    $track_response = !empty($requestData['track_response']) ? $requestData['track_response'] : '';

    $warranty_payload = !empty($requestData['warranty_payload']) ? $requestData['warranty_payload'] : '';
    $warranty_response = !empty($requestData['warranty_response']) ? $requestData['warranty_response'] : '';

    $dripcell_payload = !empty($requestData['dripcell_payload']) ? $requestData['dripcell_payload'] : '';
    $dripcell_response = !empty($requestData['dripcell_response']) ? $requestData['dripcell_response'] : '';
    ...
    if ($triggerTrackapi == 1) {
        $updateLeadData = "UPDATE auto_rocket_test SET track_payload='$track_payload',track_response= '$track_response',triggerTrackapi='$triggerTrackapi',track_preferredtimeofcall='$track_preferredtimeofcall' WHERE phone1='$phone1'";
    ...
    if ($triggerWarrantyapi == 1) {
        $updateLeadData = "UPDATE auto_rocket_test SET warranty_payload='$warranty_payload',warranty_response='$warranty_response',triggerWarrantyapi='$triggerWarrantyapi',warranty_preferredtimeofcall='$warranty_preferredtimeofcall' WHERE phone1='$phone1'";
        //file_put_contents('./updateleadlog_View' . time() . '.log',$updateLeadData, FILE_APPEND);
        if ($conn->query($updateLeadData) === TRUE) {
            $LeadResponse = array(
                "status" => "SUCCESS",
    ...
    if ($dripcellContactUpload) {
        $updateLeadData = "UPDATE auto_rocket_test SET dripcell_payload='$dripcell_payload',dripcell_response='$dripcell_response',dripcellContactUpload='$dripcellContactUpload' WHERE phone1='$phone1'";
...
$endpoints["updateTrackData"] = function (array $requestData): void {
    ...
    $updateTrackData = "UPDATE auto_rocket_test SET track_response= '$track_response', isTrackApiFired = '1' WHERE phone1='$phone1'";
    file_put_contents('./updateTrackData' . time() . '.log', $updateTrackData, FILE_APPEND);
    if ($conn->query($updateTrackData) === TRUE) {
        $LeadResponse = array(
            "status" => "SUCCESS",
            "code" => 1,
            "status_message" => 'Leads Updated',
...
$endpoints["scheduleTask"] = function (array $requestData): void {
    ...
    if ($triggerTrackapi == 1) {
        ...
        $query = "SELECT * FROM auto_rocket_test WHERE triggerTrackapi = '1' and phone1 = '$phone1' ";
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                ...
                $estTime->modify('+5 minutes');
                $executeTrackApiAt = $estTime->format("Y-m-d H:i:s");

                $updateScheduleData = "UPDATE auto_rocket_test SET scheduletrackApi='1',executeTrackApiAt='$executeTrackApiAt' WHERE phone1='$phone1'";
                ...
                if ($conn->query($updateScheduleData) === TRUE) {
                    $LeadResponse = array(
                        "status" => "SUCCESS",
                        "code" => 1,
                        "status_message" => 'Track Schedule Updated',
                    );
                    echo json_encode($LeadResponse);
                    die();
                } else {
                    $LeadResponse = array(
                        "status" => "ERROR",
                        "code" => 2,
                        "status_message" => 'Failed Track Schedule!',
                    );
                    echo json_encode($LeadResponse);
                    die();
                }
