...
$endpoints["addData"] = function (array $requestData): void {
    ...
    if ($result->num_rows > 0) {
        ...
        $query = "INSERT INTO track_rocket (funnel_id,sid,aff_sub,aff_sub2,aff_sub3,track_payload,track_response,city,optindate,optinurl,firstname,lastname,phone1,acceptterms,offer_id,preferredtimeofcall)
    VALUES('$funnel_id','$sid','$aff_sub','$aff_sub2','$aff_sub3','$track_payload','$track_response','$city','$optindate','$optinurl','$firstname','$lastname','$phone1','$acceptterms','$offer_id','$preferredtimeofcall')";
        //echo $insertQuery;
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    }
    //END
...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    $auto_preferredtimeofcall = !empty($requestData['auto_preferredtimeofcall']) ? $requestData['auto_preferredtimeofcall'] : 'N/A';
    ...
    if ($triggerAutoApi == 1) {
        $updateLeadData = "UPDATE track_rocket SET auto_payload='$auto_payload',auto_response='$auto_response',auto_preferredtimeofcall= '$auto_preferredtimeofcall',triggerAutoApi='$triggerAutoApi' WHERE phone1='$phone1'";
