...
$endpoints["addData"] = function (array $requestData): void {
    ...
    $motor_payload = $requestData['motor_payload'];
    $motor_response = $requestData['motor_response'];
    ...
    $check_duplicate = "SELECT id FROM motor_rocket WHERE phone1='$phone1'";
    $result = $conn->query($check_duplicate);

    if ($result->num_rows > 0) {
        // Record exists, perform UPDATE
        $query = "UPDATE motor_rocket SET  motor_payload='$motor_payload',motor_response='$motor_response' WHERE phone1='$phone1'";
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    } else {
        // Record does not exist, perform INSERT
        $query =  "INSERT INTO motor_rocket (funnel_id,sid,aff_sub,aff_sub2,aff_sub3,motor_payload,motor_response,optindate,optinurl,firstname,lastname,phone1,acceptterms,offer_id,preferredtimeofcall)
    VALUES('$funnel_id','$sid','$aff_sub','$aff_sub2','$aff_sub3','$motor_payload','$motor_response','$optindate','$optinurl','$firstname','$lastname','$phone1','$acceptterms','$offer_id','$preferredtimeofcall')";
        //echo $insertQuery;
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    }
    //END
...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    if ($dripcellContactUpload == 1) {
        $updateLeadData = "UPDATE motor_rocket SET dripcell_payload='$dripcell_payload',dripcell_response='$dripcell_response',dripcellContactUpload='$dripcellContactUpload' WHERE phone1='$phone1'";
