...
$endpoints["addData"] = function (array $requestData): void {
    ...
    $sid = $requestData['sid'];
    $loan_payload = $requestData['loan_payload'];
    $loan_response = $requestData['loan_response'];
    ...
    $underdebtreview = $requestData['underdebtreview'];
    $acceptterms = $requestData['acceptterms'];
    $offer_id = $requestData['offer_id'];
    $preferredtimeofcall = $requestData['preferredtimeofcall'];

    // Check if the record exists
    $check_duplicate = "SELECT id FROM loan_rocket WHERE phone1='$phone1'";
    $result = $conn->query($check_duplicate);

    if ($result->num_rows > 0) {
        // Record exists, perform UPDATE
        $query = "UPDATE loan_rocket SET  loan_payload='$loan_payload',loan_response='$loan_response' WHERE phone1='$phone1'";
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    } else {
        // Record does not exist, perform INSERT
        $query = "INSERT INTO loan_rocket (funnel_id,sid,aff_sub,aff_sub2,aff_sub3,loan_payload,loan_response,city,optindate,optinurl,underdebtreview,firstname,lastname,phone1,acceptterms,offer_id,preferredtimeofcall)
    VALUES('$funnel_id','$sid','$aff_sub','$aff_sub2','$aff_sub3','$loan_payload','$loan_response','$city','$optindate','$optinurl','$underdebtreview','$firstname','$lastname','$phone1','$acceptterms','$offer_id','$preferredtimeofcall')";
        //echo $query;
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    }
    //END
...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    if ($dripcellContactUpload == 1) {
        $updateLeadData = "UPDATE loan_rocket SET dripcell_payload='$dripcell_payload',dripcell_response='$dripcell_response',dripcellContactUpload='$dripcellContactUpload' WHERE phone1='$phone1'";
