...
$endpoints["addData"] = function (array $requestData): void {
    ...
    $sid = $requestData['sid'];
    $health_payload = $requestData['health_payload'];
    $health_response = $requestData['health_response'];
    ...
    $check_duplicate = "SELECT id FROM health_rocket WHERE phone1='$phone1'";
    $result = $conn->query($check_duplicate);

    if ($result->num_rows > 0) {
        // Record exists, perform UPDATE
        $query = "UPDATE health_rocket SET  medical_aid_payload='$health_payload',medical_aid_response='$health_response' WHERE phone1='$phone1'";
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    } else {
        // Record does not exist, perform INSERT
        $query = "INSERT INTO health_rocket (funnel_id,sid,aff_sub,aff_sub2,aff_sub3,medical_aid_payload,medical_aid_response,city,optindate,optinurl,firstname,lastname,phone1,acceptterms,offer_id,preferredtimeofcall)
    VALUES('$funnel_id','$sid','$aff_sub','$aff_sub2','$aff_sub3','$health_payload','$health_response','$city','$optindate','$optinurl','$firstname','$lastname','$phone1','$acceptterms','$offer_id','$preferredtimeofcall')";
        //echo $insertQuery;
        //file_put_contents('./insertleadlog_View' . time() . '.log', $query, FILE_APPEND);
    }
    //END
...
$endpoints["updateData"] = function (array $requestData): void {
    ...
    if ($dripcellContactUpload) {
        $updateLeadData = "UPDATE health_rocket SET dripcell_payload='$dripcell_payload',dripcell_response='$dripcell_response',dripcellContactUpload='$dripcellContactUpload' WHERE phone1='$phone1'";
