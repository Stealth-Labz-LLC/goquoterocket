...
if (!empty($requestData['loanApi']) && $requestData['loanApi'] === 'Yes') {
    ...
    if ($loanApiResult) {
        importDataToDripcell($data, $tag1 = 'QR', $tag2 = 'Loans');
    }
}
...
function loanApiCall($data, $base_url, $updateTime, $preferredtimeofcall)
{
    ...
    // print_r($loanPayload);
    // die(); 
    if ($loanPayload['age'] === '18-60' && $loanPayload['income'] !== 'R0 - R5,000') {
        $curl = curl_init();
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => http_build_query($loanPayload),
        ...
        if ($healthResponseData->code !== -2) {
            ...
            $leadPayload = array(
                ...
                'sid' => '25335',
                'api_payload' => json_encode($loanPayload),
