...
if (!empty($requestData['lifeApi']) && $requestData['lifeApi'] === 'Yes') {
...
if (!empty($requestData['lifeApi']) && $requestData['lifeApi'] === 'Yes') {
    $lifeApiResult = lifeApiCall($data, $base_url, $updateTime, $preferredtimeofcall);
    if ($lifeApiResult) {
        importDataToDripcell($data, $tag1 = 'QR', $tag2 = 'Life');
    }
}

function lifeApiCall($data, $base_url, $updateTime, $preferredtimeofcall)
{
    //Insurance api call
    $lifePayload = array(
        'returnjson' => 'yes',
        ...
        // 'channel' => 'JMAff',
        // 'leadsource' => 'JMLSAUET',
        'incomebracket' => !empty($data['income']) ? $data['income'] : 'N/A',
        'age' => !empty($data['age']) ? $data['age'] : 'N/A',
        'employed' => !empty($data['employed']) ? $data['employed'] : 'N/A',
        'acceptterms' => 'true',
    ...
    if ($lifePayload['employed'] == 'true') {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => http_build_query($lifePayload),
        ...
        curl_close($curl);
        $lifeResponseData = json_decode($response);
        if ($lifeResponseData->response === 'OK' && !empty($lifeResponseData->leadId)) {
            $postback_data = [
                "couponCodes" => [
                    "qr_life"
                ],
                ...
                "externalId" => "$lifeResponseData->leadId",
                "customParams" => [
            ...
            curl_setopt_array($stlbzCurl, array(
                ...
                CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                CURLOPT_CUSTOMREQUEST => 'POST',
                CURLOPT_POSTFIELDS => json_encode($postback_data, JSON_UNESCAPED_UNICODE),
                CURLOPT_HTTPHEADER => array(
        ...
        if ($lifeResponseData->code !== -2) {
            ...
            $leadPayload = array(
                ...
                'optinurl' => $base_url,
                'optindate' => $updateTime,
                'api_payload' => json_encode($lifePayload, JSON_UNESCAPED_UNICODE),
...
function importDataToDripcell($data, $tag1, $tag2)
{
    ...
    $dripcellPayload = [
        [
            ...
            'c4' => 'quote-rocket',
            'c5' => !empty($data['employed']) ? $data['employed'] : '',
